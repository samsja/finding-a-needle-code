...
class RelationNet(torch.nn.Module):
    ...
    def _concat_features(
        ...
        features_cat_left = torch.cat((features_supports, features_queries), dim=4)
    ...
    def forward(
        ...
        features_cat_right = features_cat_right.view(-1, *features_cat.shape[-3:])


        features_cat_left_shape = features_cat_right.shape
        features_cat_left = features_cat_left.view(-1, *features_cat.shape[-3:])
