...
def get_features_and_label(dataloader, feature_extractor, device):
    ...
    with torch.no_grad():
        ...
        for idx, batch in enumerate(tqdm(dataloader, disable=False)):
            ...
            url = url + [
                dataloader.dataset.data[idx[0].int().item()] for idx in batch["id"]
            ]
...
def plot_two_sided(df):
    ...
    img = tf.Images(img_split, img_full)
...
def get_features_labels_on_rare_class(
    dataset, class_to_search_on, feature_extractor, device
):
    with torch.no_grad():
        url = []
        features_val = []
        ...
        for class_ in class_to_search_on:
            for idx in dataset.get_index_in_class(class_):
                features_val.append(
                    feature_extractor(dataset[idx]["img"].to(device).unsqueeze(dim=0))
                )
