...
if __name__ == "__main__":
    ...
    list_file_name_train = get_file_name_from_folder(
        root_dir="/staging/thesis_data_search/data/patches",
        exclude_class=torch.arange(201, 313),
    )
    list_file_name_val = get_file_name_from_folder(
        root_dir="/staging/thesis_data_search/data/patches",
        exclude_class=torch.arange(200),
    )

    bg_dataset = TrafficSignDatasetWrapper(list_file_name_train, transform=transform)

    eval_dataset = TrafficSignDatasetWrapper(list_file_name_val, transform=transform)
    # In[12]:
